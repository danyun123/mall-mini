...
- 关键代码必须写注释，尤其是页面事件函数、`service.ts` 业务编排函数、`store` 自定义 state、`scripts/`、构建/校验脚本、复杂分支和数据转换逻辑，注释要说明意图和边界，不只写“做了什么”
- 运行时控制台错误属于架构约束问题：页面 API 参数、路由跳转、登录态写入和小程序编译告警必须在交付前主动验证
- 多人协作时，以 `docs/project-state.md` 作为稳定项目索引；新增模块或跨模块改动后先刷新它，本地时间、分支、HEAD 和工作树状态不写入共享文档
